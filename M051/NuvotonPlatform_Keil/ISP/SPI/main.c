...
/*---------------------------------------------------------------------------------------------------------*/
/*
	using SPI0(P1.4=SS0,P1.7=CLK,P1.6=MISO0,P1.5=MOSI0) as slave
*/
#include <stdio.h>
#include <string.h>
#include "M05xx.h"
...
static __inline void SPIInit(void)
{
	...
	/* Configure SPIx as a slaver, 32-bit transaction */
...
int32_t main()
{
    ...
    if(((PORT2->PIN & (1 << 2)) == 0) || ((inpw(&SYS->RSTSRC)&0x3) == 0x00) 

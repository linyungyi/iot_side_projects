...
#Region "Control"
    ...
    <Description("Current posistion for the Slider")> _
    <Bindable(True)> _
    Public ReadOnly Property SliderPos() As Single
        Get
            Return sngSliderPos
        End Get
    End Property
    'by albert
    Private _lowPos As Single
    <Category("Appearance gTrackBar")> _
    <Description("lowest value for the Slider")> _
    <Bindable(True)> _
    Public Property LowPos() As Single
        Get
            Return _lowPos
        End Get
...
#Region "Building"
    ...
    Private Sub UpdateSlider(ByVal xPos As Integer)
        ...
        If _Orientation = Windows.Forms.Orientation.Horizontal Then
            If _lowPos <> 0 And sngSliderPos < _lowPos Then sngSliderPos = _lowPos ' by albert
        ...
        'Debug.Print(sngSliderPos.ToString)
    End Sub
...
#Region "Mouse"
    ...
    Private Sub TBSlider_MouseMove(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles Me.MouseMove
        ...
        ElseIf IsOverSlider And MouseState = eMouseState.Up Then
            ...
            'If _Orientation = Windows.Forms.Orientation.Horizontal Then
            '    tmpValue = CInt(((sngSliderPos - rectSlider.X) _
            '        / (rectSlider.Width / (_MaxValue - _MinValue))) + _MinValue)

            'Else
            '    Dim adj As Integer = 0
            '    If _MinValue < 0 Then adj = Math.Abs(_MinValue)
            '    tmpValue = ((_MaxValue + adj) - CInt(((sngSliderPos - rectSlider.Y) _
            '        / (rectSlider.Height / ((_MaxValue + adj) - (_MinValue + adj)))) _
            '        + (_MinValue + adj))) - adj

...
#define inp8(port)			(*((__I  uint8_t *) (port)))
#define inp32(port)			(*((__IO uint32_t *)(port)))
#define outp32(port,value)	(*((__IO uint32_t *)(port))=(value))
...
#define USBD_IEF         	(USBD_BASE + 0x00)			
#define USBD_EVF         	(USBD_BASE + 0x04)
#define USBD_FADDR         	(USBD_BASE + 0x08)
#define USBD_STS         	(USBD_BASE + 0x0C)
#define USBD_ATTR         	(USBD_BASE + 0x10)			
#define USBD_FLODETB     	(USBD_BASE + 0x14)
...
#define IEF_WAKEUPEN    	0x00000100		
#define IEF_WAKEUP      	0x00000008		
#define IEF_FLD         	0x00000004		
#define IEF_USB         	0x00000002		
#define IEF_BUS         	0x00000001

#define EVF_SETUP       	0x80000000		
#define EVF_EPTF5       	0x00200000		
#define EVF_EPTF4       	0x00100000		
#define EVF_EPTF3       	0x00080000		
#define EVF_EPTF2       	0x00040000		
#define EVF_EPTF1       	0x00020000		
#define EVF_EPTF0       	0x00010000		
#define EVF_WAKEUP      	0x00000008		
#define EVF_FLD         	0x00000004		
#define EVF_USB         	0x00000002		
#define EVF_BUS         	0x00000001		

#define ATTR_ENUSB      	0x00000080		
#define ATTR_RWAKEUP    	0x00000020		
#define ATTR_ENPHY      	0x00000010		
#define ATTR_TIMEOUT    	0x00000008		
#define ATTR_RESUME     	0x00000004		
#define ATTR_SUSPEND    	0x00000002		
#define ATTR_USBRST     	0x00000001		

#define CFGx_STALL_CTL  	0x00000200
#define CFGx_DSQ        	0x00000080
#define CFGx_STATE      	0x00000060
#define CFGx_ISOCH			0x00000010
#define CFGx_EPT        	0x0000000F		

#define CFGPx_STALL     	0x00000002	    
#define CFGPx_CFGP      	0x00000001	    
...
#define _DRVUSB_ENABLE_MISC_INT(u32Flags)   outp32(USBD_IEF, (u32Flags) & (IEF_WAKEUP + IEF_FLD + IEF_USB + IEF_BUS))

#define _DRVUSB_ENABLE_WAKEUP()       USBD->IEF.WAKEUP_EN = 1
#define _DRVUSB_DISABLE_WAKEUP()      USBD->IEF.WAKEUP_EN = 0
#define _DRVUSB_ENABLE_WAKEUP_INT()   USBD->IEF.WAKEUP = 1
#define _DRVUSB_DISABLE_WAKEUP_INT()  USBD->IEF.WAKEUP = 0
#define _DRVUSB_ENABLE_FLD_INT()      USBD->IEF.FLD = 1
#define _DRVUSB_DISABLE_FLD_INT()     USBD->IEF.FLD = 0
#define _DRVUSB_ENABLE_USB_INT()      USBD->IEF.USB = 1
#define _DRVUSB_DISABLE_USB_INT()     USBD->IEF.USB = 0
#define _DRVUSB_ENABLE_BUS_INT()      USBD->IEF.BUS = 1
#define _DRVUSB_DISABLE_BUS_INT()     USBD->IEF.BUS = 0

/* Respond EPx (x = 0 ~ 5) STALL packet */
#define _DRVUSB_CLEAR_EP_READY_AND_TRIG_STALL(u32EpId)   	outp32(USBD_CFGP0+(0x10*(u32EpId)), (inp32(USBD_CFGP0+(0x10*(u32EpId))) | (CFGPx_STALL | CFGPx_CFGP)))
	
/* Clear EPx (x = 0 ~ 5) In/Out ready */
#define _DRVUSB_CLEAR_EP_READY(u32EpId)  				  	outp32(USBD_CFGP0+(0x10*(u32EpId)), (inp32(USBD_CFGP0+(0x10*(u32EpId))) | CFGPx_CFGP))
...
#define _DRVUSB_SET_EVF(u32Data)    					  	outp32((&USBD->EVF), (u32Data))	

#define _DRVUSB_GET_EVF()           					  	inp32((&USBD->EVF))

/* Clear EPx (x = 0 ~ 5) Force device to response STALL	*/
#define _DRVUSB_CLEAR_EP_STALL(u32EpId)    				outp32(USBD_CFGP0+(0x10 * (u32EpId)), inp32(USBD_CFGP0+(0x10 * (u32EpId))) & ~CFGPx_STALL)
	
/* Trigger EPx (x = 0 ~ 5) Force device to response STALL */
#define _DRVUSB_TRIG_EP_STALL(u32EpId)     				outp32(USBD_CFGP0+(0x10*(u32EpId)), inp32(USBD_CFGP0+(0x10*(u32EpId))) | CFGPx_STALL)

/* Clear EPx (x = 0 ~ 5) Specify Data 0 or 1 after IN token toggle automatically after host ACK	*/
#define _DRVUSB_CLEAR_EP_DSQ(u32EpId)      				outp32(USBD_CFG0+(0x10*(u32EpId)), inp32(USBD_CFG0+(0x10*(u32EpId))) & ~CFGx_DSQ)
...
/* Set System states */
#define _DRVUSB_SET_STS(u32Data)    						outp32((&USBD->STS), (u32Data))

/* Get System states */
#define _DRVUSB_GET_STS()   								inp32((&USBD->STS))

/* Configure Set CFGPx (x = 0 ~ 5) */
#define _DRVUSB_SET_CFGP(u8CFGPNum, u32Data)    outp32(USBD_CFGP0+(0x10 * (u8CFGPNum)), (u32Data) & 0x03)

#define _DRVUSB_SET_CFGP0(u32Data)  outp32(USBD_CFGP0, (u32Data) & 0x03)

#define _DRVUSB_SET_CFGP1(u32Data)  outp32(USBD_CFGP1, (u32Data) & 0x03)

#define _DRVUSB_SET_CFGP2(u32Data)  outp32(USBD_CFGP2, (u32Data) & 0x03)

#define _DRVUSB_SET_CFGP3(u32Data)  outp32(USBD_CFGP3, (u32Data) & 0x03)

#define _DRVUSB_SET_CFGP4(u32Data)  outp32(USBD_CFGP4, (u32Data) & 0x03)

#define _DRVUSB_SET_CFGP5(u32Data)  outp32(USBD_CFGP5, (u32Data) & 0x03)
...
#define _DRVUSB_ENABLE_USB()    	outp32((&USBD->ATTR), /*0x3D0*/0x7D0)

/* Disable USB, PHY and use remote wake-up */
#define _DRVUSB_DISABLE_USB()   	outp32((&USBD->ATTR), /*0x360*/0x760)

/* Disable PHY and don't use remote wake-up */
#define _DRVUSB_DISABLE_PHY()   	outp32((&USBD->ATTR), /*0x3C0*/0x7C0)
...
typedef struct
{
	...
	int32_t  abData0[MAX_EP_ID];  /* Index is EP address	*/

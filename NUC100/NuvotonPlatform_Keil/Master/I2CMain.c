...
/*---------------------------------------------------------------------------------------------------------*/
/*
	using I2C0(GPA8,GPA9) as Master
*/
#include <stdio.h>
#include <string.h>
#include "NUC1xx.h"
...
static E_I2C_PORT port = I2C_PORT0; 
...
void I2C_Callback_Master(uint32_t status)
{
	...
	else if (status == 0x50)				/* DATA has been transmitted and ACK has been received */
	{
		...
		{
			rcvbuf[Rcv_Cnt++] = DrvI2C_ReadData(port);
			DrvI2C_Ctrl(port, 0, 0, 1, 1);
		}
		else
		{
...
BOOL I2C_MasterSendData()
{
	...
	//start signal sent; I2C->STATUS == 0x08
	//and then send SLA+W to slave
	DrvI2C_WriteData(port, Device_Addr0<<1);
	DrvI2C_Ctrl(port, 0, 0, 1, 0);//send SLA+W and receive AA
	WaitSI_Timeout();
	...
	{
		I2C->DATA = sendbuf[i];
		DrvI2C_Ctrl(port, 0, 0, 1, 0);//send data and receive AA
		WaitSI_Timeout();
...
out:
	DrvI2C_Ctrl(port, 0, 1, 1, 0);//send STOP
	cnt = 0;	
	//while( I2C->CON.STO == 1);
...
BOOL I2C_MasterRcvData()
{
	...
	//(I2C->STATUS == 0x08) || (I2C->STATUS == 0x10)
	DrvI2C_WriteData(port, Device_Addr0<<1 | 0x01);//SLA + R
	DrvI2C_Ctrl(port, 0, 0, 1, 0);//send SLA+R and receive AA
	WaitSI_Timeout();
	...
	do
	{
		DrvI2C_Ctrl(port, 0, 0, 1, 1);//rcv data and send AA
		WaitSI_Timeout();
...
static __inline void I2CInit(void)
{
	...
	/* Open I2C and I2C1, and set clock = 100Kbps */
	DrvI2C_Open(port, u32HCLK, 100000);
	

	/* Get I2C clock */
	u32data = DrvI2C_GetClock(port, u32HCLK);
	...
	DrvI2C_InstallCallback(port, I2CFUNC, I2C_Callback_Master);
#endif
	...
	DrvI2C_UninstallCallBack(port, I2CFUNC);
	
	/* Disable I2C interrupt and clear corresponding NVIC bit */
	DrvI2C_DisableInt(port);

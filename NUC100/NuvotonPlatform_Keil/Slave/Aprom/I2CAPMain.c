...
static void I2C_SlaveRcvSendData()
{
	...
	if((I2C->STATUS == 0xA8) || (I2C->STATUS == 0xB0))/* SLA+R has been received and ACK has been returned */
	{
		...
		{
			I2C->DATA = sendbuf[i];
			DrvI2C_Ctrl(port, 0, 0, 1, 1);//send data and receive AA
			//while( I2C->CON.SI == 0);
			WaitSI_Timeout();
...
static __inline void I2CInit(void)
{
	...
	DrvI2C_SetAddress(port, 0, 0x36, 0);
	...
	DrvI2C_InstallCallback(port, I2CFUNC, I2C_Callback_Slave);
#endif

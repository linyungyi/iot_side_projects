...
/*---------------------------------------------------------------------------------------------------------*/
/*
	using I2C0(GPA8(SDA),GPA9(SCL)) as slave
*/
#include <stdio.h>
#include <string.h>
#include "NUC1xx.h"
...
#if USE_INTERRUPT
static volatile BOOL I2C0_Read = FALSE;
static uint8_t Send_Cnt0 = 0;
static uint8_t Rcv_Cnt0 = 0;
static volatile uint8_t EndFlag0 = 0;
#endif
...
{	
	...
	{
		if (Rcv_Cnt0 < (PACKET_SIZE-1))
		{
			rcvbuf[Rcv_Cnt0++] = DrvI2C_ReadData(I2C_PORT0);
			DrvI2C_Ctrl(I2C_PORT0, 0, 0, 1, 1);
		}
		else
		{
...
static void I2C_SlaveRcvSendData()
{
	...
	do{
		...
		DrvI2C_Ctrl(I2C_PORT0, 0, 0, 1, 1);
		while( I2C0->CON.SI == 0);
	}while((I2C0->STATUS != 0xA8) && (I2C0->STATUS != 0xB0)
			&& (I2C0->STATUS != 0x60) && (I2C0->STATUS != 0x68));//wait SLA+R/SLA+W again
	
	if((I2C0->STATUS == 0xA8) || (I2C0->STATUS == 0xB0))/* SLA+R has been received and ACK has been returned */
	{
		//printf("start sent\n");
		...
		{
			I2C0->DATA = sendbuf[i];
			DrvI2C_Ctrl(I2C_PORT0, 0, 0, 1, 1);//send data and receive AA
			//while( I2C0->CON.SI == 0);
			WaitSI_Timeout();
			
			////Timeout or NACK received, break and don't resent
			if((I2C0->CON.SI == 0) || (I2C0->STATUS != 0xB8))
				break;
			i++;
		}while(i<PACKET_SIZE);
	}
	else if((I2C0->STATUS == 0x60) || (I2C0->STATUS == 0x68))/* SLA+W has been received and ACK has been returned */
	{
		//printf("start recv\n");
		bI2CDataReady = TRUE;
		...
		do
		{
			...
			WaitSI_Timeout();
			//printf("i %d\n", i);
			if((I2C0->CON.SI == 0) || (I2C0->STATUS != 0x80))
			{
				bI2CDataReady = FALSE;
				break;
			}
...
static __inline void I2CInit(void)
{
	...
	DrvI2C_SetAddress(I2C_PORT0, 0, 0x36, 0);

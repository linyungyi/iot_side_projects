...
uint32_t g_apromSize; /* For FMC usage */
uint32_t g_SecurityLockBit = 1;
...
void UsbCfg(void)
{
    ...
    _DRVUSB_SET_CFGP0(EPT_CFGP);                            /* Clear EP0 CTRL IN Ready Flag */
    ...
    _DRVUSB_SET_CFGP1(EPT_CFGP);                            /* Clear EP1 CTRL OUT Ready Flag */
	...
	_DRVUSB_SET_CFGP2(EPT_CFGP);                            /* Clear EP2 BULK IN Ready Flag */
	...
	_DRVUSB_SET_CFGP3(EPT_CFGP);                            /* Clear EP3 BULK OUT Ready Flag */
...
void UsbFdt(void)
{
    uint8_t u8FLDET = USBD->FLODET.FLODET;
...
int32_t UsbSetup(void)
{
    ...
    _DRVUSB_SET_CFGP0(CFGPx_CFGP);
    _DRVUSB_SET_CFGP1(CFGPx_CFGP);
...
void UsbUsb(uint32_t u32INTSTS)
{
	...
	{	
		...
		{	
            ...
            _DRVUSB_SET_CFGP(0, (CFGPx_STALL | CFGPx_CFGP));
			_DRVUSB_SET_CFGP(1, (CFGPx_STALL | CFGPx_CFGP));
		}
	}
    else if(u32INTSTS & EVF_EPTF0)
	{	
	    /* Ctrl In */
        _DRVUSB_SET_EVF(EVF_EPTF0);
		
        UsbInAck();
	}
    else if(u32INTSTS & EVF_EPTF1)
	{	
	    /* Ctrl Out */
		_DRVUSB_SET_EVF(EVF_EPTF1);
        
        UsbOutAck();
	}
    else if(u32INTSTS & EVF_EPTF2)
	{	
	    /* Bulk In */
		_DRVUSB_SET_EVF(EVF_EPTF2);
        
        g_u32OutToggle = u32STS;
		UsbBulkInAck();
	}
    else if(u32INTSTS & EVF_EPTF3)
	{	
	    /* Bulk Out */
		_DRVUSB_SET_EVF(EVF_EPTF3);
		...
		else
		{
			...
			{	
			    /* stall */
				g_bCBWInvalid = TRUE;
                _DRVUSB_SET_CFGP(2, (CFGPx_STALL | CFGPx_CFGP));
				_DRVUSB_SET_CFGP(3, (CFGPx_STALL | CFGPx_CFGP));
			}
		}
	}
	else
...
void udcFlashInit(void)
{
    ...
    /* To check APROM Size */
    while (FMC_Read(u32Addr, &u32Data) == 0)
    {
        u32Addr <<= 1;
    }
    ...
    if (u32Addr >= 0x20000)
    {
        FMC_Read(Config0, &u32Data);
        
        /* Check if Data Flash Enable Function is enabled */
        if ((u32Data & BIT0) == 0)
        {
            u32Addr = inp32(DFBADDR);
            
            /* To avoid Data Flash Base Address > 0xFFFFF */
            if (u32Addr >= 0xFFFFF)
            {
                u32Addr = 0x1F000;
            }    
    ...
    FMC_Read(Config0, &u32Data);
    g_SecurityLockBit = u32Data & BIT1;
    g_apromSize       = u32Addr;  // For FMC usage     

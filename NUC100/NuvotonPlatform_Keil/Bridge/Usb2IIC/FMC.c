...
int FMC_ReadID(unsigned int cmd, unsigned int * data)
{
...
void EraseAP(BOOL bAprom, unsigned int addr_start, unsigned int addr_end)
{
    ...
    if(APROM_END > 0x20000 || APROM_END < 0x1000)//avoid config1 value from error
    	APROM_END = 0x20000;
...
void ReadID(void)
{
    unsigned int CMID, DID;
   
    FMC_ReadID(1, &CMID);
    FMC_ReadID(0, &DID);
    FMC_ReadID(1, &CMID);

    //printf("CID DID 0x%x 0x%x\n", CMID, DID);
